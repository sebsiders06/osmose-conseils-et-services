...
export function SiteExperience({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    function initMobileCoverFlow() {
      ...
      document.querySelectorAll<HTMLElement>("[data-cover-flow-clone]").forEach((el) => {
        el.remove();
      });

      type SimpleGroup = { container: HTMLElement; cards: HTMLElement[] };

      const containers: SimpleGroup[] = Array.from(document.querySelectorAll<HTMLElement>(COVER_FLOW_CONTAINER_SELECTOR))
        .map((container) => {
